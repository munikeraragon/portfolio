export const DockerIcon: React.FC = () => {
	return (
		<svg version='1.1' viewBox='0 0 512 512' className='h-14 w-14' xmlns='http://www.w3.org/2000/svg'>
			<g id='_x39_7-docker'>
				<g>
					<g>
						<rect height='43.193' fill='#099CEC' width='47.506' x='229.999' y='147.225' />
						<rect height='42.69' fill='#099CEC' width='47.506' x='229.999' y='199.115' />
						<rect height='43.193' fill='#099CEC' width='47.506' x='173.869' y='147.225' />
						<rect height='42.691' fill='#099CEC' width='47.507' x='286.2' y='199.043' />
						<rect height='43.625' fill='#099CEC' width='47.506' x='229.999' y='94.975' />
						<rect height='43.193' fill='#099CEC' width='47.505' x='117.738' y='147.225' />
						<rect height='42.691' fill='#099CEC' width='47.506' x='173.869' y='199.043' />
						<path
							d='M476.441,219.095c-10.35-6.972-34.21-9.487-52.537-6.036     c-2.371-17.249-12.003-32.27-29.538-45.782l-10.062-6.685l-6.685,10.062c-13.225,19.98-16.817,52.896-2.658,74.602     c-6.252,3.377-18.545,7.978-34.786,7.689H27.757c-6.252,36.511,4.169,83.945,31.624,116.501     c26.663,31.551,66.623,47.579,118.872,47.579c113.124,0,196.851-52.104,236.021-146.76c15.381,0.288,48.584,0.073,65.618-32.483     c1.078-1.797,4.743-9.488,6.107-12.291L476.441,219.095z'
							fill='#099CEC'
						/>
						<polygon
							points='109.186,199.043 109.113,199.043 61.68,199.043 61.68,241.734 109.186,241.734    '
							fill='#099CEC'
						/>
						<rect height='42.691' fill='#099CEC' width='47.505' x='117.738' y='199.043' />
					</g>
				</g>
			</g>
			<g id='Layer_1' />
		</svg>
	)
}
