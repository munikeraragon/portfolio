...
const Work: React.FC<WorkProps> = ({ className = '' }) => {
	return (
		<div id='work' className={`${className} flex flex-col w-full bg-white text-black min-h-screen`}>
			<p className='text-3xl font-medium text-center mb-8 mt-28'>Work</p>
			<div className='flex flex-col max-w-6xl mx-auto items-center justify-center content-center'>
				<div className='grid grid-cols-2 w-full max-w-4xl flex-1 mt-10'>
					{projects.map(({ href, icon }, index) => (
						<>
							{index % 2 !== 0 && <div></div>}
							<Link href={href}>
								<a>
									<div className='flex justify-center mx-12 w-72 shadow rounded py-4 bg-gray-50 p-4'>
										{icon}
									</div>
								</a>
							</Link>
